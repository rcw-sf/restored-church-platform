...
async function pushAdditionsToPushpay(
  firebaseAdmin: FirebaseAdmin,
): Promise<void> {
  const env = getEnvironment();
  ...
  for (const doc of snapshot.docs) {
    ...
    const payload: PushpayIndividualPayload = {
      ...
      membership_date: data.membershipStartDate,
...
async function pushEditsToPushpay(firebaseAdmin: FirebaseAdmin): Promise<void> {
  const env = getEnvironment();
  const db = firebaseAdmin.firestore();
...
export async function pushToGoogleSheets(
  ...
  const takeawayRows: TakeawaySheetRow[] = takeawaysSnapshot.docs.map((doc) => {
    ...
    return {
      ...
      ministry: data.ministry || "",
      bibleTalk: data.bibleTalk || "",
      reasonForFallaway: data.reasonForFallaway || "",
